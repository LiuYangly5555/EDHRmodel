...
%This program is used to simulate the sap flow over time in HA2

%--------------------------------
% Select one of the following four to set soil characteristic parameters
SWCr=0.065; SWCs=0.41; a=0.075; n=1.89; Ks=106.1; l=-1; SWCfc=0.1218; % Sandy loam
% SWCr=0.045; SWCs=0.43; a=0.145; n=2.68; Ks=712.8; l=-1; SWCfc=0.07; % Sand
% SWCr=0.078; SWCs=0.43; a=0.036; n=1.56; Ks=24.96; l=-1; SWCfc=0.2421;% Loam
% SWCr=0.034; SWCs=0.46; a=0.016; n=1.37; Ks=6; l=-1; SWCfc=0.3534;  % SIlt
%--------------------------------
...
%--------------------------------
% Calculate plant axial, circuferential, and radial resistance (HA2)
...
%--------------------------------
% Load leaf and meteorological factors
load('Hleaf.mat'); % load leaf water potential, cm
load('RH'); % load air relative humidity, %
load('T'); % load air temperature, ℃
load('Va'); %load wind speed, m s-1
%--------------------------------
% Set the total number of days to simulate
day=5;
%--------------------------------
% Running the simulation with time
sim=Flow_HA2(SWCr, SWCs, a,n, Ks, l, SWCfc, ...
    D1, D2, S, R, l_s, l_d, ln_s, ln_d, ...
     Hleaf, RH, T, Va, day); % in the model running, "i" represents each 10min
sim.SWC1(end)=[]; sim.SWC2(end)=[];  % remove redundant values
Graphdata(sim, day) % display data
%--------------------------------
%Export data to excel
sim_table=struct2table(sim);
writetable(sim_table, 'HA2.xlsm', 'sheet', 1)
