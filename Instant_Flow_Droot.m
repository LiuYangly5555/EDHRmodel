...
% vessel diameter of HA1 and HA2
...
Droot=1*10^-4:1*10^-4:200*10^-4; % set root vessel diameter from 1 μm to 200 μm***
...
Q=Ins_Flow_Droot_HA1(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2); % Solve the instantaneous flow in the network
...
R.Rr_s=rr_fineroot./SAr_s; % radial resistance of shallow root, MPa s cm-3
R.Rr_d=rr_fineroot./SAr_d; % radial resistance of shallow root, MPa s cm-3
Q=Ins_Flow_Droot_HA2(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2); % Solve the instantaneous flow in the network
Qc_HA2=Q.p1; % circumferential flow within shallow root

%--------------------------------
% draw the figure
plot(Droot*100, Qc_HA1, Droot*100, Qc_HA2);
xlabel("Droot (μm)");ylabel("Qc (cm3 h-1)");
