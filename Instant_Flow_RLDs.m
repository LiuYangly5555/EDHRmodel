...
% This program is used to solve the instantaneous flow under different root
% length density of shallow layer in HA1 and HA2
...
RLD_s=0.01:0.01:1.2; % set root length density of shallow layer from 0 to 1.2 cm cm-3***
...
Q=Ins_Flow_RLD_HA1(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2); % Solve the instantaneous flow in the network
...
R.Rr_s=rr_fineroot./SAr_s; % radial resistance of shallow root, MPa s cm-3
R.Rr_d=rr_fineroot./SAr_d; % radial resistance of shallow root, MPa s cm-3
Q=Ins_Flow_RLD_HA2(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2); % Solve the instantaneous flow in the network
Qc_HA2=Q.p1; % circumferential flow within shallow root

%--------------------------------
% draw the figure
plot(RLD_s, Qc_HA1, RLD_s, Qc_HA2);
xlabel("RLDs (cm cm-3)");ylabel("Qc (cm3 h-1)");
