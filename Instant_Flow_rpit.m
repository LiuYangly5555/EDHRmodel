...
% This program is used to solve the instantaneous flow under different pit
% resistance in HA1 and HA2
...
rpit=0:10*10^-2:1200*10^-2; % , set pit resistance from 0 to 1200 MPa s m-1***
...
Q=Ins_Flow_rpit_HA1(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2); % Solve the instantaneous flow in the network
...
R.Rr_s=rr_fineroot./SAr_s; % radial resistance of shallow root, MPa s cm-3
R.Rr_d=rr_fineroot./SAr_d; % radial resistance of shallow root, MPa s cm-3
Q=Ins_Flow_rpit_HA2(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2); % Solve the instantaneous flow in the network
Qc_HA2=Q.p1; % circumferential flow within shallow root

%--------------------------------
% draw the figure
plot(rpit*100, Qc_HA1, rpit*100, Qc_HA2);
xlabel("rpit (MPa s m-1)");ylabel("Qc (cm3 h-1)");
