function obj= Ins_Flow_Lv_HA1(SWC1, SWC2, H1, H2, R, l_s, l_d, ln_s, ln_d, SWCr, SWCs, n, Ks, l, D1, D2)
...
for i=1:1:a
    ...
    eqns = [-q1*(2*Rrs1+2*Rr_s+Ra_r1(1, i))+p1*Rc_r+q2*(2*Rrs1+2*Rr_s+Ra_r1(1, i))==0,...
        -(q1+p1)*(Ra_r1(1, i)+Ra_s1(1, i)/2)+p3*Rc_s+(q3-p2)*(Ra_s2(1, i)/2+Ra_r2(1, i)/2)-p2*Rc_r_2+(q2-p1)*(Ra_r1(1, i)+Ra_r1_2(1, i)/2)-p1*Rc_r==0,...
        -q3*(Rrs2+Rr_d+Ra_r2(1, i)/2)-p2*Rc_r_2-(q2-p1+p2)*(Rrs1_2+Rr_s_d+Ra_r1_2(1, i)/2)==0,...
        (q3-p2-p3)*(Ra_s2(1, i)/2+Ra_b2(1, i))-p3*Rc_s-(q1+p1+p3)*(Ra_s1(1, i)/2+Ra_b1(1, i))==0,...
        (H2-D1-D2/2)-q3*(Rrs2+Rr_d+Ra_r2(1, i)/2)-(q3-p2)*(Ra_r2(1, i)/2+Ra_s2(1, i)/2)-(q3-p2-p3)*(Ra_s2(1, i)/2+Ra_b2(1, i))-Hleaf==0,...
        (H2-D1-D2/2)+(q2-p1+p2)*(Rrs1_2+Rr_s_d+Ra_r1_2(1, i)/2)+(q2-p1)*(Ra_r1(1, i)+Ra_r1_2(1, i)/2)+q2*(2*Rrs1+2*Rr_s+Ra_r1(1, i))-(H1-D1/2)==0,...
        (H1-D1/2)-q1*(2*Rrs1+2*Rr_s+Ra_r1(1, i))-(q1+p1)*(Ra_r1(1, i)+Ra_s1(1, i)/2)-(q1+p1+p3)*(Ra_s1(1, i)/2+Ra_b1(1, i))-Hleaf==0];
